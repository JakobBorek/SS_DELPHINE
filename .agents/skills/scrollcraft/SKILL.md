...
> **Codex adapter notes (added at install):**
> - This skill was authored for Claude Code. Tool names in frontmatter (`AskUserQuestion`, `Read`, `Edit`, `Bash`) map to your own equivalents; ask the user directly instead of `AskUserQuestion`.
> - `scripts/kie.mjs` calls the KIE.ai image/video API and **costs money**. Do NOT run it unless the user explicitly authorizes it and `KIE_AI_API_KEY` is set. Prefer your own image generation.
> - `scripts/shoot.mjs` / `doctor.mjs` launch Chrome via project-local `playwright-core`. Use your native browser tooling instead unless playwright is installed here.
> - The design method, engine, templates, page grammars, feeling curve and accessibility guidance are fully portable — use those directly.
